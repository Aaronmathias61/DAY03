...
//for loop to print Id, name and email.
  for(let i=0;i<arr.length;i++){console.log(arr[i].id,arr[i].name,arr[i].email);}
//for in loopprint Id, name and email.
for(var i in arr){console.log(arr[i].id,arr[i].name,arr[i].email);}
//for of loop the value of element can be changed here the value of ID was increased by 1
for (var element of arr) {console.log(element.name,parseInt(element.id)+1);}
// For each loop to print names.
arr.forEach((ar)=>(console.log(ar.name)));// For each loop to print name